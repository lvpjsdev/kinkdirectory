  ...
  <UModal v-model:open="isOpen" size="2xl" :dismissible="false" fullscreen>
    ...
    <template #body>
      <UTabs v-model="activeTab" :items="tabs" class="w-full">
        ...
        <template #selections>
          <div v-if="activeList" class="p-6 space-y-4">
            <div class="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
              <p class="font-medium">
                Number of selections: {{ Object.keys(activeSelections).length }}
              </p>
            </div>
        ...
        <template #stats>
          <div v-if="kinkStats" class="p-6 space-y-6">
              ...
              <div class="flex items-end gap-2 mt-4 justify-center">
                <div v-for="(count, rating) in kinkStats.byRating" :key="rating" class="flex flex-col items-center w-14">
                  ...
                  <div class="text-xs mt-1 font-medium">
                    {{ ratingLabels[Number(rating) as keyof typeof ratingLabels] }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 p-4 rounded-lg shadow-sm">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div
                  v-for="(count, position) in kinkStats.byPosition" :key="position"
                ...
                >
                  ...
                  <p class="text-xs mt-1">
                    {{ Math.round(count / (kinkStats.total || 1) * 100) }}%
                  </p>
                </div>
              </div>
            </div>
